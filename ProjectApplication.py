# -*- coding: utf-8 -*-
"""
Created on Tue May 18 08:53:38 2023

@author: Sagie Zafrani
"""
from tkinter import *
...
DATA_PATH = 'household_power_consumption\household_power_consumption.txt'
...
def main():
    ...
    model_selection_btn = Button(root, text="Choose a model", width=13, font=deafault_font,
                                 command=lambda: model_menu_window(root), bg="#1988ee")
    model_selection_btn.pack(pady=20)
    save_data_btn = Button(root, text="Move data file", width=13, font=deafault_font, command=save_data_window,
                           bg="#22a515")
    save_data_btn.pack(pady=20)

    explore_data_btn = Button(root, text='Explore data', width=13, font=deafault_font, command=data_window,
                              bg="#d81111")
...
def save_data_window():
    ...
    select_folder_btn = Button(window, text="Choose a folder", font=deafault_font, command=lambda: select_dir(e),
                               bg="#eb9c1e")

    e.grid(row=1, column=0, pady=20, sticky=E + W)
    select_folder_btn.grid(row=1, column=1, sticky=E + W)

    continue_btn = Button(window, text="Continue", font=deafault_font, command=lambda: save_data(window, e.get()),
                          bg="#19e029")
...
def model_menu_window(parent):
    ...
    mulitvariate_btn = Button(window, text="Multivarite model", height=4,
                              command=lambda: actions_menu_window(window, 'Multi'), font=deafault_font, bg="#1988ee")
    mulitvariate_btn.place(bordermode=OUTSIDE, width=200, height=100, x=325, y=175)

    univariate_btn = Button(window, text="Univariate model", height=4,
                            command=lambda: actions_menu_window(window, 'Uni'), font=deafault_font, bg="#22a515")
...
def actions_menu_window(parent, model_type):
    ...
    back_btn.place(bordermode=OUTSIDE, width=70, height=30, x=20, y=5)
    if model_type == 'Uni':
        train_model_btn = Button(window, text="Train the model", font=deafault_font,
                                 command=lambda: train_model(window, 'Uni'))
    else:
        train_model_btn = Button(window, text="Train the model", font=deafault_font,
                                 command=lambda: train_model(window, 'Multi'))
    train_model_btn.place(bordermode=OUTSIDE, width=300, height=100, x=150, y=25)

    # train_model_btn.pack(pady=20)

    if model_type == 'Multi':
        test_model_btn = Button(window, text="Test the model", font=deafault_font,
                                command=lambda: select_series_to_predict(window, model_type))
    else:
        test_model_btn = Button(window, text="Test the model", font=deafault_font,
                                command=lambda: test_model(window, model_type))
    test_model_btn.place(bordermode=OUTSIDE, width=300, height=100, x=150, y=150)
    # test_model_btn.pack(pady=20)

    test_multi_forecast_btn = Button(window, text="Multiforecast with the model", font=deafault_font,
                                     command=lambda: select_multi_forecast_options(window, model_type))
...
def select_series_to_predict(parent, model_type):
    ...
    w.pack(pady=25)
    btn = Button(window, text="Test", font=deafault_font,
                 command=lambda: test_model(window, model_type, variable.get()))
    btn.pack(pady=25)
...
def select_multi_forecast_options(parent, model_type):
    ...
    if model_type == 'Multi':
        ...
        btn = Button(window, text='Forecast', font=deafault_font,
                     command=lambda: validate_forecast_len(window, model_type, e.get(), variable.get()))
    else:
        btn = Button(window, text='Forecast', font=deafault_font,
                     command=lambda: validate_forecast_len(window, model_type, e.get()))
    btn.pack(pady=30)
...
def data_window():
    ...
    show_data_btn = Button(window, text='The data format',
                           command=lambda: display_image_window(r'Images\data_table.png'), font=deafault_font)
    show_data_btn.pack()

    show_gap_disturbution = Button(window, text='Global active power disturbution',
                                   command=lambda: display_data_disturbution('Global_active_power'), font=deafault_font)
    show_gap_disturbution.pack(pady=25)

    show_voltage_disturbution = Button(window, text='Voltage disturbution',
                                       command=lambda: display_data_disturbution('Voltage'), font=deafault_font)
    show_voltage_disturbution.pack(pady=25)

    show_gi_disturbution = Button(window, text='Global intensity disturbution',
                                  command=lambda: display_data_disturbution('Global_intensity'), font=deafault_font)
...
def display_data_disturbution(column):
    df = pd.read_csv(DATA_PATH, sep=';', parse_dates={'date_time': ['Date', 'Time']}, infer_datetime_format=True,
                     index_col='date_time', na_values=['nan', '?'])
...
def save_data(parent, path):
    ...
    if os.path.exists(path):
        ...
        if len(files_in_dir) == 0:
            move(os.path.join(os.getcwd(), 'household_power_consumption.txt'),
                 os.path.join(path, r'data\household_power_consumption.txt'))
            global DATAPATH
        ...
        error_message = Label(parent, text=f"The path {path} does not exist, please enter a valid path",
                              font=deafault_font)
